...
// NOTE: all of following magic calculations are LLM generated and I have absolutely no idea how do they work, but at least they work :)
...
const generateOutline = (coords: { w: number; d: number }[]): Point[] => {
  ...
  const outlineSegments: { p1: Point; p2: Point }[] = [];
  ...
  if (path.length > 2) {
    for (let i = 0; i < path.length; i++) {
      const p_prev = path[(i + path.length - 1) % path.length];
      const p_curr = path[i];
      ...
      if (v_in.x * v_out.y - v_in.y * v_out.x !== 0) {
        corners.push(p_curr);
      }
    }
  } else {
...
const createBlobPath = (
  ...
  if (len < 3) return "";
  const getPixelPos = (p: Point) => ({
    x: p.x * STEP - CELL_GAP / 2 - offsetX,
    y: p.y * STEP - CELL_GAP / 2 - offsetY,
  });
  const pathPoints: {
    p_before: Point;
    p_after: Point;
  ...
  for (let i = 0; i < len; i++) {
    ...
    const t_before = {
      x: pos_curr.x - v_in_n.x * cornerRadius,
      y: pos_curr.y - v_in_n.y * cornerRadius,
    };
    const t_after = {
      x: pos_curr.x + v_out_n.x * cornerRadius,
      y: pos_curr.y + v_out_n.y * cornerRadius,
    };
    const p_before = {
      x: t_before.x + v_in_p.x * padding,
      y: t_before.y + v_in_p.y * padding,
    };
    const p_after = {
      x: t_after.x + v_out_p.x * padding,
      y: t_after.y + v_out_p.y * padding,
    };
    pathPoints.push({ p_before, p_after, sweepFlag, radius: cornerRadius });
  }
  for (let i = 0; i < len; i++) {
    const current = pathPoints[i];
    const prev = pathPoints[(i + len - 1) % len];
    if (i === 0) {
      path.push(`M ${prev.p_after.x} ${prev.p_after.y}`);
    }
    path.push(`L ${current.p_before.x} ${current.p_before.y}`);
    path.push(
      `A ${current.radius} ${current.radius} 0 0 ${current.sweepFlag} ${current.p_after.x} ${current.p_after.y}`,
    );
  }
  path.push("Z");
  return path.join(" ");
...
const generateCalendarLayout = (year: number): YearLayout => {
  ...
  if (dates.length === 0)
    return { year, months: [], totalWidth: 0, totalHeight: 0 };
  ...
  const months: MonthLayout[] = monthlyCells
    .map((cells, monthIndex) => {
      ...
      const labelWeek =
        firstCompleteWeekIndex !== undefined
      ...
      const svgWidth =
        (maxWeek - minWeek + 1) * STEP + 2 * BLOB_PADDING + CELL_GAP;
      const svgHeight = 7 * STEP + 2 * BLOB_PADDING + CELL_GAP;
      const pathData = createBlobPath(outline, {
      ...
      return {
        ...
        cells,
        label: {
          text: new Date(Date.UTC(year, monthIndex)).toLocaleString("en-US", {
            month: "short",
          }),
          x: labelWeek * STEP,
        },
  ...
  const maxWeek =
    allCells.length > 0 ? getGridPos(allCells[allCells.length - 1].date).w : 0;
...
const CalendarGrid = ({
  ...
  const logsMap = new Map(
    logs.map((log) => [toYYYYMMDD(new Date(log.date)), log.value]),
  );
    ...
    [
      ...layout.months.map((month) =>
        h<State>(
          "svg",
          {
            key: `blob-${month.monthIndex}`,
            class: "absolute",
            style: {
              left: `${month.svg.x}px`,
              top: `${month.svg.y + MONTH_LABEL_GUTTER}px`,
              width: `${month.svg.width}px`,
              height: `${month.svg.height}px`,
              overflow: "visible",
            },
          },
          h<State>("path", {
            d: month.svg.path,
            fill: HSLToString(color.monthBackground),
          }),
        ),
      ),

      ...layout.months.flatMap((month) =>
        month.cells.map((cell) => {
          ...
          const canToggle =
            isEditing && new Date(new Date().setUTCHours(0, 0, 0, 0)) >= date;
          ...
          return h("div", {
            key: dateStr,
            class: `absolute rounded-sm ${canToggle ? "cursor-pointer hover:opacity-75" : ""}`,
            style: {
              left: `${w * STEP}px`,
              top: `${d * STEP + MONTH_LABEL_GUTTER}px`,
              width: `${CELL_SIZE}px`,
              height: `${CELL_SIZE}px`,
            ...
            onclick: (state: State) => {
              ...
              if (habit.isBoolean) {
                ...
                const rawValue = prompt(
                  `Enter value for ${date.toDateString()}:`,
                  String(logValue),
                );
                if (rawValue === null) return state;
                ...
                if (!isNaN(value) && value >= 0) {
                  return [
                    ...
                    [
                      UpsertHabitLogFx,
                      { habitId: habit.id, date: dateStr, value, token },
                    ],
                  ];
                } else {
      ...
      ...layout.months.map((month) =>
        h<State>(
          "div",
          {
            key: `label-${month.monthIndex}`,
            class: "absolute text-xs",
            style: {
              color: HSLToString(color.text),
              left: `${month.label.x}px`,
              top: "0px",
            },
          },
...
export const HabitCard = ({
  ...
  activeHabitMenuId,
  editingHabit,
}: HabitCardProps): VNode<State> => {
  const displayHue =
    isEditing && editingHabit ? editingHabit.colorHue : habit.colorHue;
  const color = getColor(displayHue);

  const todayStr = toYYYYMMDD(new Date());
  const todayLog = habit.logs.find(
    (log) => toYYYYMMDD(new Date(log.date)) === todayStr,
  );
      ...
      return [
        ...
        [
          UpdateHabitFx,
          {
            habitId: habit.id,
            name: newName,
            colorHue: state.editingHabit.colorHue,
            token,
          },
        ],
      ];
    ...
    [
          ...
          ? h<State>(
              ...
              [
                h("input", {
                  name: "habit-name",
                  class:
                    "text-xl font-bold py-2 bg-transparent focus:outline-none w-full",
                  style: {
                ...
                h<State>(
                  ...
                  [
                    h<State>("div", { class: "flex items-center gap-2" }, [
                      h<State>(
                        "label",
                        {
                          ...
                          class: "text-sm",
                          style: { color: HSLToString(color.text) },
                        },
                        text("Hue (0-360):"),
                      ),
                      h("input", {
                        ...
                        max: "360",
                        value: editingHabit
                          ? editingHabit.colorHue
                          : habit.colorHue,
                        class:
                          "bg-transparent w-20 text-center py-2 focus:outline-none",
                        style: {
                          borderBottom: `1px solid ${HSLToString(color.cellBorder)}`,
                        },
                        oninput: (state: State, event: Event) => {
                          const rawValue = (event.target as HTMLInputElement)
                            .value;
                          const value = parseInt(rawValue, 10);
                          if (!isNaN(value) && value >= 0 && value <= 360) {
                            return UpdateEditingHabitField(state, {
                              field: "colorHue",
                              value,
                            });
                          }
                    ...
                    Button(
                      { type: "submit", class: "py-1 px-3 text-sm" },
                      text("Save"),
                    ),
                  ],
                ),
          ...
          ? h("div", { class: "flex items-center gap-2" }, [
                      ...
                      onclick: (state: State) => [
                        ...
                        [
                          UpsertHabitLogFx,
                          {
                            habitId: habit.id,
                            date: todayStr,
                            value: wasLoggedToday ? 0 : 1,
                            token,
                          },
                        ],
                      ],
                      title: wasLoggedToday
                        ? "Mark as not done for today"
              ...
              h("div", { class: "relative" }, [
                h(
                  ...
                  {
                    class:
                      "p-2 rounded-full hover:bg-white/20 text-white hover:text-white transition-colors flex",
                ...
                Popup(
                  ...
                  Menu({}, [
                    ...
                    MenuItem(
                      {
                        onclick: (state: State) => [
                          CloseHabitMenu(state),
                          [DeleteHabitFx, { habitId: habit.id, token }],
                        ],
                        class:
      ...
      h<State>("div", { class: "relative" }, [
        h<State>(
          ...
          {
            class:
              "flex flex-nowrap flex-row-reverse w-full overflow-x-scroll overflow-y-hidden no-scrollbar items-end gap-4",
          },
          years.map((year) =>
            h<State>(
              "div",
              {
                key: String(year),
                class:
                  "flex flex-col items-center gap-2 flex-shrink-0 px-4 md:px-8",
              },
              [
                CalendarGrid({
                  year,
                  habit,
                  logs: logsByYear[year] || [],
                  isEditing,
                  token,
                  color,
                }),
