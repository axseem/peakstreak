...
export const UserListItem = (user: PublicUser, onclickAction?: (state: State) => State): VNode<State> =>
  h("a", {
    href: `/@${user.username}`,
    class: "flex items-center gap-4 p-4 rounded-lg hover:bg-neutral-800 transition-colors w-full",
    ...
    Avatar({ src: user.avatarUrl, username: user.username, sizeClass: "w-12 h-12" }),
