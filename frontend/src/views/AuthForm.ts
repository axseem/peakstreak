...
export const AuthForm = (props: { isLogin: boolean }) => (state: State): VNode<State> => {
  ...
  const onsubmit = (state: State, event: SubmitEvent): Dispatchable<State, any> => {
    ...
    if (props.isLogin) {
      const loginPayload = { identifier: data.username, password: data.password };
  ...
  return h("div", { class: "w-full h-screen flex flex-col items-center justify-center gap-4" }, [
    h("form", { class: "flex flex-col gap-4 w-80", onsubmit }, [
      Input({ label: "Username", id: "username", type: "text", required: true }),
