...
const LeaderboardUserCard = ({
  ...
  return h(
    ...
    {
      class:
        "flex flex-col gap-4 p-6 rounded-2xl bg-neutral-900/50 border border-neutral-800/50",
    },
    [
      h(
        "div",
        {
          class:
            "flex flex-row items-start sm:items-center justify-between gap-4",
        },
        [
          h("div", { class: "flex items-center gap-4 flex-grow min-w-0" }, [
            h(
              ...
              {
                class:
                  "text-2xl font-bold text-neutral-500 w-8 text-center flex-shrink-0",
              },
              text(`${rank}`),
            ),
            h(
              "a",
              {
                ...
                onclick: (s: State, e: Event) => {
                  ...
                  return [
                    s,
                    [NavigateFx, { path: `/@${entry.user.username}` }],
                  ];
                },
              },
              [
          ...
          h(
            ...
            {
              class:
                "flex flex-col items-end flex-shrink-0 self-end sm:self-center",
            },
            [
              h(
                "span",
                { class: "text-xl font-bold" },
                text(entry.totalLoggedDays),
              ),
              h(
                "span",
      ...
      entry.habits.length > 0 &&
        h(
          ...
          {
            class:
              "flex flex-col gap-8 pt-4 mt-4 border-t border-neutral-800/50",
          },
  ...
  return h("div", { class: "flex flex-col gap-8 w-full" }, [
    ...
    users.length === 0
      ? h(
          ...
          text("The leaderboard is empty. Start tracking habits!"),
        )
      : h(
          "div",
          { class: "flex flex-col gap-6" },
          users.map((entry, index) =>
            LeaderboardUserCard({ entry, rank: index + 1 }),
          ),
        ),
  ]);
};
